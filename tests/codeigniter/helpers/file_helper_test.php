...
class File_helper_Test extends CI_TestCase {
	...
	public function test_read_file()
	{
		...
		$file = org\bovigo\vfs\vfsStream::newFile('my_file.txt')->withContent($content)->at($this->_test_dir);

		$this->assertEquals($content, read_file(org\bovigo\vfs\vfsStream::url('my_file.txt')));
	}
	...
	public function test_octal_permissions()
	{
		...
		$file = org\bovigo\vfs\vfsStream::newFile('my_file.txt', 0777)
			->withContent($content)
	...
	public function test_get_mime_by_extension()
	{
		...
		$this->assertEquals('text/plain', get_mime_by_extension(org\bovigo\vfs\vfsStream::url('my_file.txt')));

		// Test a mime with an array, such as png
		$file = org\bovigo\vfs\vfsStream::newFile('foo.png')->at($this->_test_dir);

		$this->assertEquals('image/png', get_mime_by_extension(org\bovigo\vfs\vfsStream::url('foo.png')));

		// Test a file not in the mimes array
		$file = org\bovigo\vfs\vfsStream::newFile('foo.blarfengar')->at($this->_test_dir);

		$this->assertFalse(get_mime_by_extension(org\bovigo\vfs\vfsStream::url('foo.blarfengar')));
	}
	...
	private function _test_get_file_info($vals)
	{
		...
		$info = get_file_info(org\bovigo\vfs\vfsStream::url('my_file.txt'), $vals);
	...
	// --------------------------------------------------------------------
	 ...
	 public function test_write_file()
	 {
		...
		$file = org\bovigo\vfs\vfsStream::newFile('write.txt', 0777)
			->withContent('')
			->lastModified(time() - 86400)
			->at($this->_test_dir);

		$this->assertTrue(write_file(org\bovigo\vfs\vfsStream::url('write.txt'), $content));
	 }
